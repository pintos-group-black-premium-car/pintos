...
thread_hold_the_lock(struct lock *lock)
{
  ...
  list_insert_ordered (&thread_current ()->locks, &lock->lockelem, lock_cmp_priority, NULL);
...
thread_update_priority (struct thread *t)
{
  ...
  if (!list_empty (&t->locks))
  {
    ...
    lock_priority = list_entry (list_front (&t->locks), struct lock, lockelem)->max_priority;
...
{
  if (t->status == THREAD_BLOCKED && t->sleep_endtick > 0)
  {
      t->sleep_endtick--;
      if (t->sleep_endtick == 0)
      {
          thread_unblock(t);
      }
  }
